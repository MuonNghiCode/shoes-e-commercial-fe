    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="p-0 max-w-md w-full right-0 left-auto ml-auto h-screen flex flex-col">
        ...
        <DrawerHeader className="sticky top-0 z-10 bg-white flex flex-row items-center justify-between px-6 py-4 border-b">
          ...
          <DrawerClose asChild>
            <button className="text-2xl text-gray-400 hover:text-black px-2" aria-label="Close">×</button>
          </DrawerClose>
        </DrawerHeader>
        {/* Content */}
        <div className="flex-1 overflow-y-auto px-6 py-4 bg-white">
          {cart.length === 0 ? (
            <div className="text-lg text-gray-500 text-center py-6">Your cart is empty.</div>
          ) : (
              ...
              {cart.map((item) => (
                <li key={item.id + item.size} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-base truncate">{item.name} <span className="ml-2 text-xs text-gray-500">(Size {item.size})</span></div>
                    <div className="text-sm text-gray-500 mt-1">
                      <span className="font-semibold text-black">{item.quantity}</span>
                      <span className="mx-1">×</span>
                      <span className="font-semibold text-black">{item.price.toLocaleString()}đ</span>
                    </div>
                  </div>
                  <button
        ...
        <DrawerFooter className="sticky bottom-0 z-10 bg-white px-6 pt-4 pb-8 border-t">
          <div className="flex justify-between items-center mb-5">
            ...
            <span className="text-xl font-bold text-black">{total.toLocaleString()}đ</span>
          </div>
